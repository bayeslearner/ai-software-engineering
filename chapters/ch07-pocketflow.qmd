...
class Summarize(Node):
    ...
    def exec(self, text):                  # <2>
        return call_llm(f"Summarize: {text}")  # Pure computation

    def post(self, shared, prep_res, exec_res):  # <3>
        shared["summary"] = exec_res       # Write to store
        return "default"                   # <4>
```
1. **`prep` is the read phase** — it pulls just what this node needs out of the shared store and hands it on; `exec` never touches `shared` directly.
2. **`exec` is the pure-compute phase** — it sees only `prep`'s return value, so the LLM call is isolated and safe to retry or test without side effects.
3. **`post` is the write phase** — given the original `shared`, the prep result, and the exec result, it writes outputs back and chooses what happens next.
4. **The return value is the edge label** — `"default"` names which outgoing transition to follow; returning a different string (e.g. `"search"`) is how a node steers the flow.
...
class ChunkDocs(Node):                                        # <1>
    ...
    def post(self, shared, prep_res, exec_res):
        shared["chunks"] = exec_res                           # <3>
```
1. **Plain `Node`** — chunking is a single operation on a single input (the raw text). The *output* is a list, but the input isn't, so this is not the batch case.
2. **`exec()` does the work** — keeps I/O (`shared` access) in `prep`/`post`, computation in `exec`. The retry/error semantics PocketFlow promises only apply to what's inside `exec()`.
3. **Write to `shared` in `post()`** — never write to `shared` from `prep` or `exec`. The next node will read `shared["chunks"]` (see callout below — this implicit cross-node contract is what §7.8 calls out as PocketFlow's main liability).

```python
class EmbedChunks(BatchNode):                                 # <1>
    def prep(self, shared):
        return shared["chunks"]                               # <2>

    def exec(self, chunk):                                    # <3>
        return get_embedding(chunk)

    def post(self, shared, prep_res, exec_res):
        shared["embeddings"] = exec_res                       # <4>
```
1. **`BatchNode` (not `Node`)** — signals that this node iterates `exec()` over a list. The input is *already* a list of chunks; we want one embedding call per chunk, not one call with all of them.
2. **`prep` returns the list** — `shared["chunks"]` was written by `ChunkDocs.post()` in the previous step. **This is the implicit contract**: nothing in the type system says `EmbedChunks` needs `chunks` to exist. Rename a key upstream and this breaks silently at runtime. The Burr section (@sec-prod-burr) replaces this string-keyed handoff with declared `reads`/`writes`.
3. **`exec()` receives ONE chunk at a time** — PocketFlow handles the iteration. This is the "map" in map/reduce.
4. **`exec_res` is the list of ALL results** — after all `exec()` calls complete, PocketFlow collects them into a list and passes it to `post()`. This is the "reduce."
...
app = (
    ApplicationBuilder()
    .with_actions(answer_question=answer_question.bind(llm=llm))     # <4>
    .with_transitions()                                              # <5>
...
print(state["answer"])
```
0. **`llm` is your LLM client** — Anthropic, OpenAI, whatever has a `.complete()` method. Shown as a placeholder here.
...
4. **`with_actions(name=action.bind(...))`** registers actions and pre-binds their non-state dependencies. `.bind()` works like `functools.partial`. The kwarg key (`answer_question=`) becomes the action's name in the graph; positional form (`with_actions(answer_question)`) also works and uses `func.__name__`.
...
6. **`run(halt_after=[...])`** returns the `(action_obj, result_dict, final_state)` triple and stops after the named action completes. Burr also has `step`, `iterate`, and `stream_result` variants — see the next subsection.
...
class QAState(BaseModel):                                            # <1>
    query: str
    context: list[dict] = Field(default_factory=list)                # <2>
    next_action: str | None = None                                   # <3>
    search_term: str | None = None                                   # <3>
    answer: str | None = None

@action.pydantic(reads=["query", "context"], writes=["answer"])      # <4>
def synthesize(state: QAState, llm) -> QAState:                      # <5>
    state.answer = llm.complete(                                     # <6>
...
app = (
    ...
    .with_typing(PydanticTypingSystem(QAState))                      # <7>
...
print(state.data.answer)                                             # <8>
```
1. **State is a Pydantic `BaseModel`.** Fields are typed; defaults are real Python values, not magic strings.
2. **`context: list[dict]`** holds search results — each entry is `{"term": ..., "result": ...}`. The `synthesize` action below treats the whole list as opaque context for the LLM; the search action in the worked example is what appends to it.
3. **`next_action` and `search_term` are unused by this minimal example** but declared up front so the schema is stable across sections. The `decide` action in the worked example writes them; the FSM transitions read `next_action` to choose the next edge.
4. **`@action.pydantic`** is the typed sibling of `@action`. It still declares `reads`/`writes`, but the action body sees a typed object.
5. **`state: QAState`** — the IDE autocompletes `state.query`, `state.context`, `state.answer`. A typo (`state.querry`) is a static-analysis error before you run.
6. **Field mutation looks normal.** Burr's runtime captures the change and produces a new immutable snapshot under the hood; you write idiomatic Python.
7. **`with_typing(PydanticTypingSystem(QAState))`** attaches the type system to the application. Burr now validates state transitions against the schema.
8. **`state.data` returns the typed object** after `run()`. `state.data.answer` is what you get; `state["answer"]` still works but you've left the typed path.
...
We use the same `QAState` introduced in @sec-prod-burr-pydantic — five fields, one canonical schema for the whole chapter:

```python
class QAState(BaseModel):
    query: str
    context: list[dict] = Field(default_factory=list)                # <1>
    next_action: str | None = None                                   # <2>
    search_term: str | None = None                                   # <2>
    answer: str | None = None
```
1. **`context` accumulates** — each search appends a dict `{"term": ..., "result": ...}`; the decide step reads the whole list.
2. **Routing fields.** `decide` writes `next_action` (`"search"` or `"answer"`) and `search_term`; the transitions in @sec-prod-burr-transitions read `next_action` to pick the edge.
...
@action.pydantic(reads=["query", "context"],
                 writes=["next_action", "search_term"])      # <1>
...
```"""
    ...
    parsed = yaml.safe_load(raw.split("```yaml")[1].split("```")[0])  # <2>
    state.next_action = parsed["action"]                     # <3>
...
def answer(state: QAState, llm) -> QAState:
    ...
    return state
```
1. **The decorator *is* the contract** — `reads`/`writes` declare exactly which state fields this action may touch, so Burr can validate the whole graph before a single LLM call runs.
2. **Pull the YAML back out of the model's reply** — the prompt asked for a fenced ```` ```yaml ```` block, so this splits on the fences to grab just the payload before parsing it.
3. **Routing fields drive the FSM** — `decide` writes `next_action`; the transitions read it to choose the next edge, which is how this one action steers the loop.
4. **Append, don't mutate** — building a *new* `context` list (rather than `.append`) keeps each step an immutable snapshot, which is what makes Burr's replay and fork-from-state work.
...
## Beyond the Worked Example: Burr's Feature Set {#sec-prod-burr-features}

The Q&A agent used Burr's spine — actions, typed state, transitions, persistence. Production agents need more than a spine. This section tours the features that turn it into a deployable system, each grounded in [Burr's documentation](https://burr.dagworks.io/). None of them change the mental model; they hang off the same actions-and-transitions machine you already have.

### Lifecycle hooks: cross-cutting logic without touching actions {#sec-prod-burr-hooks}
...
class TracingHook(PreRunStepHook, PostRunStepHook):                  # <1>
    ...
    def post_run_step(self, *, state: State, action: Action,
                      ...
                      exception: Exception, **future_kwargs: Any):   # <3>
        if exception:
...
app = ApplicationBuilder().with_actions(...).with_hooks(TracingHook()).build()  # <4>
```
1. **A hook subclasses one or more hook protocols.** `PreRunStepHook` + `PostRunStepHook` are the two you reach for most; the family also includes `PostApplicationCreateHook`, `PreRunApplicationHook`/`PostRunApplicationHook` (around the whole run), and `PostEndStreamHook` for streaming.
2. **`pre_run_step` fires before each action.** Always accept `**future_kwargs` — Burr adds keyword arguments over time, and this keeps your hook forward-compatible.
3. **`post_run_step` fires after each action** and receives the `result`, the `sequence_id` (the snapshot number you'd fork from), and any `exception`. This is exactly where a cost meter, an OpenTelemetry span, or a Datadog metric goes.
4. **`with_hooks(...)` registers them.** The actions stay pure; the cross-cutting concern lives in one place. This is the idiomatic way to bolt observability vendors onto Burr without editing a single action body.

### Streaming: token-by-token without losing typed state {#sec-prod-burr-streaming}
...
class StreamingAnswer(StreamingAction):
    async def stream_run(self, state: State, **kwargs) -> AsyncGenerator[dict, None]:
        buffer = []
        async for delta in llm.astream(state["query"]):   # <1>
            buffer.append(delta)
            yield {"response": delta}                      # <2>
        yield {"response": "".join(buffer)}               # <3>
    ...
    def update(self, result: dict, state: State) -> State:
        return state.update(answer=result["response"])    # <4>
```
1. **`stream_run` is an async generator.** Each `yield` is a partial result pushed to the caller.
2. **Intermediate chunks** drive the UI — print them, send them over a websocket.
3. **The last yield is the complete value** that `update` will commit. Burr distinguishes "still streaming" from "done" by the generator finishing.
4. **`update` runs once at the end** and produces the immutable snapshot — so streaming output and the replayable state contract coexist. The typed sibling is `@streaming_action.pydantic` (with `state_input_type`, `state_output_type`, `stream_type`), which streams *partial Pydantic objects* via Instructor.

Consume it with the `stream_result` variant of `run` (the third sibling alongside `run` and `iterate` mentioned in @sec-prod-burr — §7.9.1):

```python
action, container = app.stream_result(halt_after=["generate"], inputs={"query": q})  # <1>
for partial in container:        # <2>
    print(partial["response"], end="", flush=True)
_, _, state = container.get()    # <3>
```
1. **`stream_result` hands back a *container*, not a finished result** — execution is paused mid-action so you can pull output as it's produced.
2. **Each `partial` is an incremental chunk** — iterating the container yields tokens as the model emits them, which is what lets you print a live, streaming response.
3. **`container.get()` returns the committed state once the stream closes** — only after streaming finishes do you get the final, persisted typed state; the streamed text and the durable snapshot stay separate.

### Human-in-the-loop: halt, inspect, resume {#sec-prod-burr-hitl}
...
if get_human_approval(proposed):                                   # <2>
    ...
    action, result, state = app.run(halt_after=["execute_trade"],
                                    inputs={"approved": True})      # <3>
```
1. **`halt_before=["execute_trade"]`** runs the loop right up to — but not into — the named action, and returns control. `halt_after` is its sibling for "stop once this completes."
2. **The gate is your code, not Burr's.** Burr's job is to *pause cleanly with the full state available*; the approval channel (a web form, a Slack button, a CLI prompt) is yours.
3. **Resuming is just another `run`** with `inputs` supplying the human's answer. With persistence on, this can be a brand-new process — load the app from the snapshot (`initialize_from`, @sec-prod-burr-replay) and continue. That's what makes Burr HITL work for a deployed web service, not just a notebook.

### Observability: a built-in UI, plus OpenTelemetry {#sec-prod-burr-otel}
...
app = (
    ApplicationBuilder()
    .with_actions(...)
    .with_tracker("local", project="qa-agent")   # <1>
    .build()
)
# then, in a terminal:  `burr`  → opens the UI at http://localhost:7241
```
1. **`with_tracker("local", project=...)`** streams every run, action, and state snapshot to the local tracking store. The bundled `burr` CLI launches the dashboard — the same graph view, run timeline, state diff, and fork button from @sec-prod-burr-ui, now fed live.
...
### Testing: fixtures captured from real runs {#sec-prod-burr-testing}
...
from burr.testing import pytest_generate_tests                     # <1>

@pytest.mark.file_name("decide_search.json")                       # <2>
def test_decide_routes_to_search(input_state, expected_state):
    in_state  = state.State.deserialize(input_state)
    out_state = decide(in_state, llm=fake_llm)                     # <3>
    # exact match, or fuzzy/LLM-graded for non-deterministic fields
    assert out_state["next_action"] == expected_state["next_action"]
```
1. **Importing `pytest_generate_tests`** activates Burr's file-based parametrization.
2. **`@pytest.mark.file_name(...)`** points at a fixture holding `input_state` and `expected_state` — generated by serializing a captured snapshot, so your tests exercise real production states.
3. **Call the action directly** with a stub LLM. No builder, no transitions, no I/O — the reads/writes contract is what makes this isolation sound.

### Parallelism, briefly {#sec-prod-burr-parallel}
...
## Burr vs LangGraph {#sec-prod-burr-vs-langgraph}
...
|---|---|---|
| **Programming model** | Explicit FSM — `@action`s with declared `reads`/`writes`; edges are `(from, to, Condition)` tuples | Graph — nodes are functions; edges (incl. conditional) wire them, routing returns the next node |
...
::: {.callout-note title="The numbers move — and a few common claims are wrong"}
...
The book's path is: this chapter picks the state machine, Chapters 8–9 cover more production options, **Chapter 10 turns the survey into a decision** — two postures, five axes, and two more runtimes (Pydantic AI, Microsoft Agent Framework) — the Ralph loop in Chapter 11 wraps the loop around it, and Chapters 13–14 scale the loop into a fleet and a career.

::: {.callout-tip title="The property of Burr that Chapter 10 turns into a design"}
`GraphBuilder().build()` returns a **value**. That sounds like an implementation detail and is actually the discriminating feature of the whole field: because a graph is a value, code can *compose one at runtime from a plan the model emitted* — which is what makes a plan inspectable, policy-checkable and costable before anything runs. @sec-rt-composed builds exactly that.
:::
