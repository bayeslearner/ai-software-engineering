...
The [Learn Claude Code course](https://learn.shareai.run/en/) continues beyond what we've covered here — adding permission systems (s07), memory (s09), task graphs (s12), background execution (s13), agent teams (s15-s16), autonomous agents (s17), and worktree isolation (s18). All 19 sessions layer onto the same 30-line loop without restructuring it. The next section makes that claim concrete: the course's final session reassembles *every* mechanism into a single file, and the core is still the loop you've already read.

## Putting It All Together: One File, Every Mechanism {#sec-build-capstone}

The course's final session — [`s20_comprehensive/code.py`](https://github.com/shareAI-lab/learn-claude-code/blob/main/s20_comprehensive/code.py) — is the payoff for everything above. It is roughly two thousand lines, but it contains no new *kind* of idea: it puts dispatch, permission, hooks, todos, subagents, skills, compaction, memory, a task graph, error recovery, background tasks, cron, teams, and MCP back into **the same loop**. This section walks the load-bearing additions the earlier sections promised but didn't show. The snippets are trimmed to their teaching core; read the source for the production edge cases.
...
For brevity this section skipped several subsystems that follow the identical "layer on, don't restructure" rule. In the source they are each a handful of functions:
...
The full file is worth reading end to end precisely *because* none of it is surprising once you've internalized the loop. Each subsystem is a small, self-contained module that the loop calls at a well-defined moment — which is the whole thesis of this chapter, demonstrated at scale.
...
- The capstone (`s20`) reassembles *every* mechanism — task graph, worktrees, teams + approval protocol, error recovery, background, cron, MCP — into one file, and the core is still the same loop
