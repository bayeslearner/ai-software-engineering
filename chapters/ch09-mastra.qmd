...
Not every tool call should execute automatically. For sensitive operations (database writes, financial transactions, external API mutations), Mastra supports tool approval:
...
const agent = new Agent({
  ...
  toolApproval: {                                                      // <1>
    deleteTool: 'required',                                            // <2>
    readTool: 'auto',                                                  // <3>
  },
})

const result = await agent.generate('Delete user 42.', {
  onToolApproval: async (toolCall) => {                                // <4>
    console.log(`Agent wants to call: ${toolCall.name}(${JSON.stringify(toolCall.args)})`)
    const approved = await askHuman('Approve this action? (y/n)')
    return approved === 'y'
  },
})
```
1. **`toolApproval`** configures per-tool approval requirements.
2. **`'required'`** — every invocation of `deleteTool` must be approved before execution.
3. **`'auto'`** — `readTool` executes immediately without approval.
4. **`onToolApproval`** is the callback that fires when approval is required. You implement the approval UX — a CLI prompt, a Slack message, a web form, whatever fits your deployment.
...
const server = new MCPServer({
  ...
  agents: { researchAgent },                                           // <1>
  tools: { weatherTool },                                              // <2>
  workflows: { approvalWorkflow },                                     // <3>
})

server.start()                                                         // <4>
```
1. **Agents become MCP tools.** Each agent is exposed as a tool that other MCP clients can invoke. The agent's instructions and capabilities are described in the tool's MCP metadata.
2. **Native tools pass through.** `createTool()` tools are exposed with their Zod schemas translated to JSON Schema for MCP compatibility.
3. **Workflows become tools.** Each workflow's input schema becomes the tool's input; the workflow's output becomes the tool's response.
4. **`start()`** begins listening for MCP connections. The transport (stdio or HTTP) is determined by configuration.
...
const searchStep = createStep({
  ...
  execute: async ({ inputData, setState, getState }) => {              // <2>
    const results = await search(inputData.query)
    const prev = getState().searchResults
    setState({ searchResults: [...prev, ...results],                   // <3>
               attempts: getState().attempts + 1 })
    return { results }
  },
})
```
...
2. **`setState` and `getState`** are available in every step's `execute` function when the workflow declares a `stateSchema`.
3. **State updates are validated.** If you try to set `attempts` to `"three"`, Zod catches it at runtime.
...
Sometimes you don't need human input — you need to wait for time to pass:
...
const waitStep = createStep({
  ...
  execute: async ({ inputData, sleep, sleepUntil }) => {
    await sleep(60 * 5)                                                // <1>
    // or: await sleepUntil(new Date('2026-06-01T09:00:00Z'))          // <2>
    const status = await checkOrderStatus(inputData.orderId)
    return { status }
  },
})
```
...
2. **`sleepUntil(date)`** pauses until a specific timestamp. Useful for scheduled operations — "process this order when the business day starts."
...
// 3. Embed the chunks
const embeddings = await embed(chunks, {                               // <3>
  model: 'openai/text-embedding-3-small',
})

// 4. Store in a vector database
const vectorStore = new PgVector({ connectionString: process.env.PG_URL })
await vectorStore.upsert('knowledge-base', embeddings)                 // <4>

// 5. Retrieve at query time
const results = await vectorStore.query('knowledge-base', {            // <5>
  queryVector: await embed('What is context engineering?'),
  topK: 5,
})
```
...
3. **`embed()`** turns text into vectors using the specified model. Mastra supports OpenAI, Cohere, and other embedding providers.
4. **Vector storage** — Mastra ships adapters for pgvector, Pinecone, Qdrant, MongoDB Atlas, and others. The interface is consistent across backends.
...
const mastra = new Mastra({
  ...
  telemetry: {
    ...
    sensitiveDataFilter: true,                                         // <2>
  },
})
```
1. **OTLP exporter** sends traces to any OpenTelemetry-compatible backend.
2. **Sensitive data filtering** strips PII and secrets from traces before export. You define what's sensitive; Mastra redacts it.
...
mastra.start()                                                         // <2>
```
1. **Server configuration** is part of the Mastra instance. CORS, middleware, and authentication are configured here.
2. **`start()`** launches the HTTP server. Agents are accessible via REST endpoints; workflows can be triggered via API calls.
...
## Guardrails and Processors {#sec-mastra-guardrails}

Production agents need safety boundaries. Mastra provides two mechanisms for controlling agent behavior at the framework level.

**Guardrails** run before or after agent responses and can reject unsafe outputs:
    ...
    input: [                                                           // <1>
      {
        ...
        execute: async ({ input }) => {
          if (containsPII(input)) {
            return { blocked: true, reason: 'Input contains PII' }
          }
    ...
    output: [                                                          // <2>
      {
        ...
        execute: async ({ output }) => {
          const safe = await moderationCheck(output)
          return { blocked: !safe, reason: 'Output flagged by moderation' }
        },
      },
    ],
  },
...
```
1. **Input guardrails** run before the LLM sees the message. Use them for PII detection, prompt injection defense, or input validation.
2. **Output guardrails** run after the LLM responds but before the response reaches the user. Use them for content moderation, compliance checks, or output validation.

When a guardrail blocks, the workflow result status is `'tripwire'` — a distinct state from `'failed'` that signals "the agent could have responded, but a safety check prevented it."

**Processors** transform messages before they reach the agent. Unlike guardrails (which block), processors modify:

```typescript
const agent = new Agent({
  id: 'processed-agent',
  processors: {
    pre: [({ message }) => sanitize(message)],                         // <1>
    post: [({ response }) => addDisclaimer(response)],                 // <2>
  },
})
```
1. **Pre-processors** clean up input — normalize formatting, strip HTML, redact sensitive fields.
2. **Post-processors** modify output — add disclaimers, format for a specific channel, translate.
